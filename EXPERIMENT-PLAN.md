...
flake's `shen-go` pin (`610ba42`), matching the fg-go model work in the `shen1` session. The two
...
The fg-go pilot now accepts an optional retained runtime receipt via
`CAPCOV_FG_GO_RUNTIME_RECEIPT`. The producer is the existing disposable
`tools/check_subscription_links.py` path in fg-go, extended at candidate
...
nix develop --no-update-lock-file --command bash -lc \
  'export PYTHONPATH="$PWD/src"; python -m unittest discover -s tests/claim_semantics -p "test_fg_go_static*.py" -t .'
```
...
### 2026-09-15 fg-go causal runtime trace follow-up

The recipient-link fixture now emits schema `capcov-fg-go-runtime-route/v2` from fg-go commit
...
typed runtime facts, rejects evidence whose producer class is not `fg-go-runtime-trace-v2`, joins
...
without conflicts. `schema_replay_v1.json` is **v1-draft** until the fg-go `export-facts` runner
...
### 2026-09-15 fg-go runtime join integrated (second fork); producer-authority collision resolved
...
branch from `3e8a5f8` and pushed `507ef19`: an optional retained fg-go runtime receipt
(`CAPCOV_FG_GO_RUNTIME_RECEIPT`, schema `capcov-fg-go-runtime-route/v2`, produced by fg-go's
...
evidence with `kind="fg-go-runtime-trace-v2"`. On the merged tree that rejected every replay
record (kind `fact`): the replay suite fell to 19 failures and 32 errors, and conversely the
runtime evidence (source first token `fg-go`) would have failed the cherry-picked
...
static corpus 18, static differential 18, evidence policy 5 all OK; fg-go pilot gate with the
