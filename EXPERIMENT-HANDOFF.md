...
| Ordering, cross-request and cross-run replay claims | `claims/replay/schema_replay_v1.json`, `experiments/claim-semantics/replay/`, `tests/claim_semantics/test_replay_*`, `.../test_target_go_replay_receipt.py` | Branch `experiment/r2-ordering`. Seven new observation relations (per-statement `php`/`go`/`model_effect_seq`, `replay_request_seq`, `php`/`go_response`, `replay_stability`) with their closures; ~20 rules add order refinement, the repeat-delete claim `repeat_delete_not_found(run, target)` and oracle stability, and `op_qualified_rt` is gated on all three. Corpus is 19 cases + 4 rejected, both kernels agreeing. Committed target-go fixture regenerated from run `eca6e7930af3` (17 relations, all seven closures true): `op_qualified(delete-issue)` supported under the four reviewer exclusions, order exercised and unviolated, oracle stable. The repeat request is in the runner's tape but not in that run, so `repeat_delete` is empty on the committed fixture; a fresh four-request run is the open item. |
