...
def evaluate_join(join: ReplayJoin, replay_root: str, *, kernels: str = "two",
    ...
    for claim in join.bundle.claims:
        rows = claim_conclusions(join.bundle, closures[0].relations, claim)
        for report in closures[1:]:
            if rows != claim_conclusions(join.bundle, report.relations, claim):
                raise AssertionError(f"{claim.id}: the kernels disagree on the claim rows ({report.backend})")
        for row in rows:
            certificate = certify(join.bundle, closures[0].relations, claim.relation, row)
            for report in closures[1:]:
                if certificate != certify(join.bundle, report.relations, claim.relation, row):
                    raise AssertionError(f"{claim.id}: certificates differ between closures ({report.backend})")
            for report in closures:
                checked = recheck(join.bundle, certificate, report.relations)
                if not checked.ok:
                    raise AssertionError(f"{claim.id}: recheck failed against {report.backend}: {checked}")
